<template>
	<view class="book-manager-container">
		<!-- 顶部管理导航 -->
		<uni-nav-bar :fixed="true" left-icon="left" title="大全" @clickLeft="handleBack">
			<template #right>
				<uni-icons type="plus" size="24" color="#1890ff" @click="showAddDialog" />
			</template>
		</uni-nav-bar>

		<!-- 搜索和筛选区域 -->
		<view class="search-filter-bar">
			<uni-search-bar v-model="searchKeyword" placeholder="搜索图书名称/ISBN" @confirm="loadBookList"
				@clear="clearSearch" radius="100" clear-button="auto" />

			<view class="filter-row">
				<picker class="filter-picker" mode="selector" :range="filterOptions.status" range-key="label"
				...
				<picker class="filter-picker" mode="selector" :range="filterOptions.categories" range-key="label"
		...
		<!-- 图书列表表格 -->
		<scroll-view scroll-y class="book-list-container" :style="{ height: scrollViewHeight }">
			...
			<view class="book-list-row" v-for="book in bookList" :key="book.id" @click="showBookDetail(book)">
				<view class="row-cell" style="width: 10%;">
					<image :src="book.cover || '/static/default-book.png'" class="book-cover" mode="aspectFit" />
				</view>
				...
				<view class="row-cell actions" style="width: 20%;">
					<uni-icons type="compose" size="20" color="#1890ff" @click.stop="editBook(book)" />
					<uni-icons type="trash" size="20" color="#f56c6c" @click.stop="confirmDelete(book.id)" />
				</view>
			</view>
		...
		<!-- 图书详情抽屉 -->
		<uni-drawer ref="detailDrawer" mode="right" :width="700" @change="onDrawerChange">
			<view class="drawer-content" v-if="currentBook">
				<!-- 详情头部 -->
				<view class="detail-header">
					<text class="detail-title">图书详情</text>
					<uni-icons type="close" size="24" color="#999" @click="closeDetail" />
				</view>

				<!-- 详情内容 -->
				<scroll-view scroll-y class="detail-body">
					<view class="detail-section">
						<image :src="currentBook.cover || '/static/default-book.png'" class="detail-cover"
							mode="aspectFit" />
				...
				<view class="detail-footer">
					<button class="action-btn status-btn" :class="{ 'offline': currentBook.status === 'online' }"
						@click="toggleBookStatus">
						{{ currentBook.status === 'online' ? '下架图书' : '上架图书' }}
					</button>

					<button class="action-btn edit-btn" @click="editCurrentBook">
						编辑图书
					</button>

					<button class="action-btn delete-btn" @click="confirmDelete(currentBook.id)">
						删除图书
					</button>
				</view>
			</view>
		...
		<uni-popup ref="bookDialog" type="dialog">
			<uni-popup-dialog :title="dialogTitle" mode="base" @confirm="submitBookForm" @close="closeDialog">
				<scroll-view scroll-y class="dialog-scroll"><!-- 滚动 -->
				<view class="dialog-form">
					<view class="form-item">
						<text class="form-label">图书ID:</text>
						<input class="form-input" v-model="bookForm.title" placeholder="请输入图书ID" />
					</view>

					<view class="form-item">
						<text class="form-label">ISBN编号:</text>
						<input class="form-input" v-model="bookForm.isbn" placeholder="请输入ISBN编号" />
					</view>
					
					<view class="form-item">
						<text class="form-label">原标题(外文书籍适用):</text>
						<input class="form-input" v-model="bookForm.title" placeholder="请输入图书原标题" />
					</view>

					<view class="form-item">
						<text class="form-label">作者:</text>
						<input class="form-input" v-model="bookForm.author" placeholder="请输入作者" />
					</view>

					<view class="form-item">
						<text class="form-label">出版社:</text>
						<input class="form-input" v-model="bookForm.publisher" placeholder="请输入出版社" />
					</view>

					<view class="form-item">
						<text class="form-label">库存:</text>
						<input class="form-input" type="number" v-model="bookForm.stock" placeholder="请输入库存数量" />
					</view>
					...
					<view class="form-item">
						...
						<image v-if="bookForm.cover" :src="bookForm.cover" class="cover-preview" mode="aspectFit" />
					</view>				
				</view>
				</scroll-view>
			</uni-popup-dialog>
		...
		<uni-popup ref="deleteDialog" type="dialog">
			<uni-popup-dialog type="warn" title="确认删除" :content="`确认删除这本图书吗？此操作不可恢复！`" @confirm="deleteBook"
				@close="closeDeleteDialog" />
		</uni-popup>
	</view>
	<uni-popup ref="versionDialog" type="dialog">
		<uni-popup-dialog :title="versionDialogTitle" mode="base" @confirm="submitVersionForm"
			@close="closeVersionDialog">
			<view class="dialog-form">
				<view class="form-item">
					<text class="form-label required">版本代码:</text>
					<input class="form-input" v-model="versionForm.book_code" placeholder="唯一版本标识" />
				</view>

				<view class="form-item">
					<text class="form-label">语言版本:</text>
					<picker mode="selector" :range="languageOptions" @change="onLanguageChange">
						<view class="form-input">
							{{ versionForm.language || '选择语言' }}
						</view>
					</picker>
				</view>

				<view class="form-item">
					<text class="form-label required">库存:</text>
					<input class="form-input" type="number" v-model="versionForm.stock" placeholder="库存数量" />
				</view>

				<view class="form-item">
					<text class="form-label required">价格:</text>
					<input class="form-input" type="number" v-model="versionForm.price" placeholder="售价" />
				</view>
			</view>
		</uni-popup-dialog>
	</uni-popup>
	...
	<!-- 在图书详情抽屉中增加版本信息部分 -->
	<view class="detail-section" v-if="currentBook">
		<view class="section-title">版本信息</view>
		<view class="version-list">
			<view v-for="version in bookVersions" :key="version.book_code" class="version-item">
				<text class="version-code">{{ version.book_code }}</text>
				<text class="version-meta">{{ version.language }} | ¥{{ version.price }} | 库存:{{ version.stock }}</text>
				<view class="version-actions">
					<uni-icons type="compose" size="20" color="#1890ff" @click.stop="editVersion(version)" />
					<uni-icons type="trash" size="20" color="#f56c6c"
						@click.stop="confirmDeleteVersion(version.book_code)" />
				</view>
			</view>
			<button class="add-version-btn" @click="showAddVersionDialog">
				<uni-icons type="plus" size="16" color="#1890ff" />
				添加版本
			</button>
		</view>
	</view>
	...
	export default {
		...
		data() {
			return {
				...
				languageOptions: ['中文', '英文', '日文', '韩文', '法文', '德文'],
				deleteId: null
			}
		},
		computed: {
		...
		methods: {
			async loadBookVersions(bookId) {
			      try {
			        const res = await uni.request({
			          url: `/api/books/${bookId}/versions`
			        });
			        if (res[1].statusCode === 200) {
			          this.bookVersions = res[1].data;
			        }
			      } catch (error) {
			    ...
			    onLanguageChange(e) {
			      this.versionForm.language = this.languageOptions[e.detail.value];
			    },
			    
			    async submitVersionForm() {
			      if (!this.validateVersionForm()) return;
			      
			      try {
			        uni.showLoading({ title: '提交中...' });
			        
			        const url = this.versionForm.book_id ? 
			          `/api/books/versions/${this.versionForm.book_code}` : 
			          '/api/books/versions';
			        
			        const method = this.versionForm.book_id ? 'PUT' : 'POST';
			        ...
			        if (res[1].statusCode === 200) {
			          ...
			          this.loadBookVersions(this.currentBook.id);
			        }
			      } catch (error) {
			    ...
			    validateVersionForm() {
				  if (!this.bookData.isbn || !this.bookData.title) {
				         uni.showToast({
				           title: 'ISBN和书名不能为空',
				           icon: 'none'
				         });
			      ...
			      if (!this.versionForm.book_code) {
			        uni.showToast({ title: '请输入版本代码', icon: 'none' });
			        return false;
			      }
			      
			      if (!this.versionForm.price || isNaN(this.versionForm.price) || this.versionForm.price <= 0) {
			        uni.showToast({ title: '请输入有效的价格', icon: 'none' });
			        return false;
			      }
			      
			      if (!this.versionForm.stock || isNaN(this.versionForm.stock) || this.versionForm.stock < 0) {
			    ...
			    confirmDeleteVersion(bookCode) {
			      uni.showModal({
			        title: '确认删除',
			        content: '确定删除这个版本吗？',
			        success: (res) => {
			          if (res.confirm) {
			            this.deleteVersion(bookCode);
			          }
			        }
			      });
			    },
			    
			    async deleteVersion(bookCode) {
			      try {
			        uni.showLoading({ title: '删除中...' });
			        const res = await uni.request({
			          url: `/api/books/versions/${bookCode}`,
			          method: 'DELETE'
			        });
...
<style scoped>
    ...
    .empty-placeholder {
      background-image: url('/static/empty-book.png');
